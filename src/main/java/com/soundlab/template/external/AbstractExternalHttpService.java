package com.soundlab.template.external;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.datatype.jsr310.JavaTimeModule;
import com.soundlab.template.exceptions.ExternalServiceException;
...
public abstract class AbstractExternalHttpService<T> {
    private static final Logger LOG = LogManager.getLogger(AbstractExternalHttpService.class);
    protected T api;
    ...
    protected AbstractExternalHttpService(String baseUrl, Callable<String> tokenGatherer, int tokenRenewalInSeconds) {
        this.api = getRetrofitClient(baseUrl).create(getApiType());
    ...
    protected void callVoid(Call call) {
        try {
            call.execute().body();
        } catch (IOException e) {
            throw new ExternalServiceException(getApiType().getSimpleName(), e.getMessage());
